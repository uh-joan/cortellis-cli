#!/usr/bin/env python3
"""Catch drugs missed by per-phase searches for an indication.

Fetches ALL drugs for an indication (no phase filter), compares against
existing phase CSVs, and writes any missing drugs to other.csv.

Usage: python3 catch_missing_drugs.py <indication_id> <landscape_dir>
...
def fetch_all_drugs(indication_id, hits=200):
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 catch_missing_drugs.py <indication_id> <landscape_dir>", file=sys.stderr)
        sys.exit(1)

    indication_id = sys.argv[1]
    landscape_dir = sys.argv[2]

    all_drugs = fetch_all_drugs(indication_id)
    ...
    # Phases to exclude (attrition / inactive)
    EXCLUDE_PHASES = {
    ...
    output = os.path.join(landscape_dir, "other.csv")
